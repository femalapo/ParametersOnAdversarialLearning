...
public class AgentControllerHigh : Agent
{
    ...
    private void Start()
    {
        ...
        observations = new float[8]; // Constant 8 for HighOb Agent
    }
    ...
    public override void CollectObservations(VectorSensor sensor)
    {
        ...
        Vector2 opponentBulletLocation = new Vector2(-2f, -2f);    // Default location to -2 if does not exist (location can be from -1 to 1)
        float bulletHeading = -2f;                                 // Default heading to -2 if does not exist (heading can be from -1 to 1)
        ...
        float[] opponentObservations = null;
        if (opponentLow != null)
        {
            opponentObservations = opponentLow.observations;
        }
        else if (opponentMedium != null)
        {
            opponentObservations = opponentMedium.observations;
        }
        else if (opponentHigh != null)
        {
            opponentObservations = opponentHigh.observations;
        }
        else
        {
            Debug.Log(gameObject.name + " [Observation] Opponent Agent not found");
        }

        // Add opponent's observations
        for (int i = 0; i < opponentObservations.Length; i++) { sensor.AddObservation(opponentObservations[i]); }
        ...
        //Debug.Log($"{gameObject.name}: {opponentBulletVisible} -- {opponentBulletLocation} -- {bulletHeading}");
    }
